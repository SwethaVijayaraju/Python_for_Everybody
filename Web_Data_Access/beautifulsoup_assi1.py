...
# Actual data: http://py4e-data.dr-chuck.net/comments_673917.html (Sum ends with 24)
# You do not need to save these files to your folder since your program will read the data directly from the URL.
# Note: Each student will have a distinct data url for the assignment - so only use your own data url for analysis.
# Data Format: The file is a table of names and comment counts.
...
ctx.verify_mode = ssl.CERT_NONE  # ctrl+space

url = input("Enter: ")
data = urllib.request.urlopen(url, context=ctx).read()
soup = BeautifulSoup(data, 'html.parser')
tags = soup('span')  # some tags - a, img, video, h1, button, table, div, etc # tags = list of objects

cmt_num = list()
count = 0
for tag in tags:
    str_num = tag.contents[0]
    try:
        num = int(str_num)
    except:
        print("Error - Cannot be an integer")
        continue
    count = count + 1
    cmt_num.append(num)
print('Count', count)
print('Sum', sum(cmt_num))

# print(tag)
# print(tag.get('class', None))
# print(tag.get('href', None))  # for 'a' - it can be href or class or id #for 'img' - it can be src or height etc
# print(tag.attrs)
# <a> href = "jshfdkbj" ...<\a>
# <img> href = "FBRDHKTRKL" ...<\img>
